...
export const mpesaStkStatus = pgEnum("mpesa_stk_status", [
	"pending",
	"success",
	"failed",
	"timeout",
...
export const smsFilterCriteria = pgEnum("sms_filter_criteria", [
	"by status",
	"by plan",
	"all members",
	"specific members",
...
export const memberMemberships = pgTable(
	...
	(table) => [
		index("idx_member_membership_member_id").using(
			"btree",
			table.memberId.asc().nullsLast().op("text_ops"),
		),
		index("idx_member_membership_membership_plan_id").using(
			"btree",
...
export const membershipPlans = pgTable(
	...
	(table) => [
		index("idx_membership_plan_name").using(
			"btree",
...
export const members = pgTable(
	...
	(table) => [
		index("idx_member_first_name").using(
			"btree",
			table.firstName.asc().nullsLast().op("text_ops"),
		),
		index("idx_member_last_name").using(
			"btree",
			table.lastName.asc().nullsLast().op("text_ops"),
		),
		index("idx_member_status").using(
			"btree",
			table.memberStatus.asc().nullsLast().op("enum_ops"),
		),
...
export const ledgerAccounts = pgTable(
	...
	(table) => [
		index("idx_accounts_code").using(
			"btree",
			table.code.asc().nullsLast().op("text_ops"),
		),
		index("idx_accounts_name").using(
			"btree",
			table.name.asc().nullsLast().op("text_ops"),
		),
...
export const expenseHeaders = pgTable(
	...
	(table) => [
		index("expense_expense_date_idx").using(
			"btree",
			table.expenseDate.asc().nullsLast().op("date_ops"),
		),
		index("expenses_expense_no_idx").using(
			"btree",
			table.expenseNo.asc().nullsLast().op("int4_ops"),
		),
		index("expenses_reference_idx").using(
			"btree",
...
export const smsBroadcasts = pgTable(
	"sms_broadcasts",
	{
		...
		message: text().notNull(),
		receipients: text().array().notNull(),
		smsBroadcastStatus: smsBroadcastStatus("sms_broadcast_status")
			.default("draft")
			.notNull(),
...
export const vwAttendanceDetails = pgView("vw_attendance_details", {
